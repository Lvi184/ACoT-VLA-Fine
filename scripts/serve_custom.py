#!/usr/bin/env python
"""
Custom serve script for our trained model that loads norm stats from baseline.
"""
import dataclasses
import enum
import logging
...
class Args:
    ...
    # Path to load norm stats from (if different from checkpoint dir).
...
def create_policy(args: Args) -> _policy.Policy:
    ...
    match args.policy:
        case Checkpoint():
            ...
            # If custom norm_stats_dir is provided, load from there
            if args.norm_stats_dir is not None:
                # Load norm stats from custom directory
                data_config = config.data.create(config.assets_dirs, config.model)
                if data_config.asset_id is None:
                    raise ValueError("Asset id is required to load norm stats.")
                norm_stats = _checkpoints.load_norm_stats(args.norm_stats_dir, data_config.asset_id)
                return _policy_config.create_trained_policy(
                    config, args.policy.dir, default_prompt=args.default_prompt, norm_stats=norm_stats
                )
            else:
                # Use default loading
                return _policy_config.create_trained_policy(
